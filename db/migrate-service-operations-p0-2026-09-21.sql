...
CREATE TABLE IF NOT EXISTS `t_staff_outing` (
  ...
  `submitted_at` datetime DEFAULT NULL COMMENT '提交审核时间（本人提交，不接受他人代录）',
  ...
  `review_remark` varchar(500) DEFAULT NULL COMMENT '审核意见（驳回必填）',
  `departed_photo_key` varchar(255) DEFAULT NULL COMMENT '出发打卡照片fileKey',
  `returned_photo_key` varchar(255) DEFAULT NULL COMMENT '返回打卡照片fileKey',
  `status` varchar(16) NOT NULL DEFAULT 'PENDING_REVIEW' COMMENT 'PENDING_REVIEW待审核/READY审核通过待出发/REJECTED已驳回/IN_PROGRESS外出中/COMPLETED已完成/CANCELLED已取消',
