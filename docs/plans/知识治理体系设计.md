...
| # | 事实 | 核实方式 | 影响 |
|---|------|---------|------|
| F1 | 全库表数**以 `db/loan-db-schema.sql` 为准**（不写死数字） | `grep -c "CREATE TABLE" db/loan-db-schema.sql` | 全库表数**一律表述为「以 `db/loan-db-schema.sql` 为准」——不写死具体数字**，不引 65（见 F23） |
| F2 | 后端源包 `com.loan.*` 共 **46 个包**（含空包 8 个） | `find -maxdepth 1 -type d` | 业务域划分依据 |
| F3 | **空包三分类**（据 `scripts/check-kb-consistency.py` 检查 3 实测，修正了初稿"8 个包均无实现"的错误）：<br>**① 真空包（真正无实现，标 ⚠️）**：`vip` `stats` `crm` —— 全仓零相关类<br>**② 位置不一致（relocated，禁止标"无实现"）**：`enums`→`engine/enums/`（Grade/StepResult 等 4 个）；`oss`→`attachment/`+`approval/`（ServiceAttachment 等；注：无独立 `*Oss*` 类，是否真是对象存储待确认）；`interceptor`→`infrastructure/interceptor/`；`job`→`lead`\|`partner`；`match`→散落 8 处（`mini/service/MiniMatchService`+`engine/service/MatchService`+`screening/`）<br>**③ 已从空包清单剔除**：`attachment` `report` `screening` `infrastructure` 等确有实现的包 | `python3 scripts/check-kb-consistency.py`（检查 3）+ 逐包 `find` 复核 | **关键区分**：索引的目的是告诉人「功能在哪」，不是「包存不存在」。标"无实现"会导致后续开发重复造轮子，故 ② 类必须标 `relocated:<实际路径>`；`implemented` 字段用**三态** `true` / `false` / `"relocated:<路径>"` |
...
| F20 | `10-历史结论与决策日志.md` **已由团队建立并持续追加**（**条目数与编号以文件实际内容为准**，追加前跑下方命令确认） | 读全文 + 逐行提取编号序列 | 本设计原拟的 `D-001` 编号格式**作废**，沿用实际 `D<n>` 格式；**新增编号 = 台账当前最大编号 + 1**（追加前跑 `grep -o "^| D[0-9-]*" docs/knowledge-base/10-历史结论与决策日志.md` 确认，见 §0.2）。曾出现 D10 重号，已修复 |
...
| F23 | **表数口径**：代码实测 68；`方案评审定稿纪要.html` 已按 D6 全改为 68（:73/:403/:522），但**其第 16 章域级明细加总仍为 65**（差 3 张未定位到域） | 读 10-日志 D6 + engineer 实测 | 索引一律写「以 `db/loan-db-schema.sql` 为准」，**不写死数字、不引 65**（规则 9），缺口由校验脚本持续暴露 |
...
| 产物 | 状态 | 本设计的调整 |
|------|------|------------|
| `docs/knowledge-base/10-历史结论与决策日志.md` | **已存在并持续追加**（**条目数与编号以文件实际内容为准**） | T03 由"新建"改为"**对齐与追加**"；编号沿用 `D<n>` 格式，**新增编号 = 台账当前最大编号 + 1（追加前 grep 确认，规则 9）**；本设计 §B.4.3 的三条已按 team-lead 指令顺延为 **D11（知识治理）/ D12（artifacts）/ D13（渠道 tabBar）**，其中 **D12 已随执行写入日志**，T03 只需补 D11、D13 |
...
> **下一条新结论编号 = 台账当前最大编号 + 1**（追加前必须跑 `grep -o "^| D[0-9-]*" docs/knowledge-base/10-历史结论与决策日志.md` 取实际最大值）。
...
| ④ 结论日志 | `10-历史结论与决策日志.md` **已由团队建立**（编号序列以文件实际内容为准）。本设计负责把它接成**强制入口**：被 `loan-knowledge`（项目级，Step 0）与 `change-safety-check`（用户级，L3 路径固化）双重先读，并在 `README.md` 三条铁律 F-1、`00-项目结构与代码地图.md` 顶部、G9 知识治理域三处设指针 |
...
6. **【规则 9】禁止写死易腐化的瞬时状态** —— **唯一真源 = 台账 `D15`，本设计不另作表述。**

   凡需表述规则 9 的场合，一律引用 `docs/knowledge-base/10-历史结论与决策日志.md` 的 **D15** 条目
   （含规则正文、三类例外、判据，以及 2026-08-31 误删 D8 恢复锚点的事故与根因）。
   **禁止在任何文档 / 技能中自行复述规则 9**——各自复述必然变形，
   而"无例外的规则"会让照做的人出错（这正是本次事故根因：错误的规范比没有规范更危险）。

   > **本设计对 D15 的唯一补充（补充，不是复述）**：
   > D15 例外②「纠错证据」在保留原值时，**必须同时标注时点**并给出当前取值命令。
   > 例：写「**D6 时点**真值为 68，纪要同期已按 D6 改为 68、不加勘误；**当前真值一律以 `db/loan-db-schema.sql` 为准**」，
   > 而不写「真值 68」——否则读者会把历史值当成现值用。
   > 已按此执行于 `.workbuddy/skills/loan-database/SKILL.md:81` 与 `loan-knowledge/SKILL.md:103`。
...
├── 00-项目结构与代码地图.md            # 保留
├── 01-角色权限模型.md                  # 保留（待修正：渠道 tabBar 与 TabBar.vue 对齐，F9）
├── 02-业务红线与编码规范.md            # 保留
├── 03-数据模型（DB schema 索引）.md    # 保留（表数以 db/loan-db-schema.sql 为准，不写死）
├── 04-后端 API 契约.md                # 保留
├── 05-前端工程要点.md                  # 保留
├── 06-业务结论沉淀索引（C1-C26）.md    # 保留（标题改 C1-C22，F15）
├── 07-沟通-上线-测试-部署清单.md       # 保留
├── 08-小程序角色功能矩阵.md            # 保留（角色技能的数据来源，审批行待修正）
├── 09-业务流程知识图谱.md / .json      # 保留
├── 10-历史结论与决策日志.md            # 【已建立】D 系列倒序（条目与编号以文件为准），修改前必查
├── 11-业务域文档索引.md                # 【新建】人读完整版「域 → 文档」大表
├── artifacts/                          # 【已归档 9 个文件】见 §A.3.1（原 5 条死链已清零，F18）
...
| **tabBar** | **结构以 `loan-mini/components/TabBar.vue` 的 `store.isChannel` 分支为准**（渠道不显示匹配 / 报告 / 服务单 tab）。⚠️ 不写死数量——`01-角色权限模型.md` 曾写"3 tab"，已确认为错（D13），**以代码为准**，01 文档待修正 |
...
- **禁止写死易腐化的瞬时状态（见台账 `D15`，含三类例外，勿自行复述）**：只写真源 + 取值命令，不写取值本身（commit hash / 台账最大编号 / 表数 / UI 数量等）
...
`10-历史结论与决策日志.md` 已建立并持续追加（**条目与编号一律以文件实际内容为准**）。**格式以其现有「追加模板」为准**，本设计初稿的 `D-001` 编号格式作废。新增条目规则：

1. **编号**：**= 台账当前最大编号 + 1**。取值命令：`grep -o "^| D[0-9-]*" docs/knowledge-base/10-历史结论与决策日志.md | head -1`（**禁止凭记忆或凭本文档写号**）。
...
5. `role-channel` 的 tabBar **指向 `TabBar.vue` 为真源、不写死数量**（D15 规则正文），并标注 01 文档的"3 tab"已确认为错（D13 例外②纠错证据，保留原值）
...
**涉及文件**
```
修改  docs/knowledge-base/10-历史结论与决策日志.md        （顶部追加 **D13 渠道 tabBar**、**D11 知识治理**；**D12 已随执行入日志，勿重复追加**。若追加时台账最大编号已变化，则按实际最大值顺延）
新建  docs/knowledge-base/11-业务域文档索引.md            （A.4 全表，人读完整版）
新建  docs/knowledge-base/index/domain-index.json        （机器可读）
```
**预期产出**：
- 在 `10-*.md` 顶部追加 **D13（渠道 tabBar 纠错）**、**D11（知识治理体系设计）** 两条（沿用实际表格格式；内容见 §B.4.3 可直接取用）。**D12（artifacts）已由工程师执行时写入，勿重复追加**
...
5. **表数口径统一**：索引中出现表数处一律写「以 `db/loan-db-schema.sql` 为准」，**不写死数字、不得出现 65**（F23 / 台账 `D15`）
...
修改  docs/knowledge-base/06-业务结论沉淀索引（C1-C26）.md （标题改 C1-C22，F15）
...
2. `01` 的渠道 tabBar 改为**与 `TabBar.vue` 一致**且注明依据（不写死数量，见台账 `D15`）；`06` 标题改 C1-C22；`08` 审批行与 B.2.3 速查矩阵完全一致（deptmgr 的 ALLOCATION 为 ❌）
...
| E4 | `06-业务结论沉淀索引（C1-C26）.md` 改名会否断裂外部引用？ | 会（README、`04`、`08`、技能都引用旧名）。建议**保留文件名、只改标题**为 C1-C22，并在文件头注明"文件名沿用旧编号，实际到 C22" | T05 的改法 |
...
| ~~E8~~ | ~~表数口径：68 vs 纪要第 16 章加总 65，差 3 张未定位~~ | ✅ **已拍板：交脚本自动核对，不单开人工轮次**（记为 **D9**）。已由校验脚本新增第 6 项检查：输出「schema 有而纪要未提及」的表名清单 + 建表语句前几行，标 `WARN`、**不计入退出码**。索引统一写「以 `db/loan-db-schema.sql` 为准」，**不写死数字**、不引 65（F23 / 规则 9）。**缺口本身由脚本持续暴露，不再列为未决** | 已决 |
...
| **v1.4** | 2026-08-31 | 落实 team-lead 三项裁决：<br>① **例外条款获批**并写入 §A.2 原则 6：保留 team-lead 采纳的判据原文；明确 **D8 恢复锚点就地记在 D8 条目、不另设存放点**；补充「纠错证据须标注时点 + 同时给当前取值命令」的写法要求（D13 / D6 / D0-2）<br>② **修复 4 处 `68 表`**：采用保语义方案（保留"契约基线/契约真源"定位，只把数字换成真源指引），未用 sed 删除法<br>③ **发现同类漏网 12 处**：上一轮修复只匹配了字面 `68 表`，遗漏 `**68** 张` 变体，共 9 个文件 12 行。已修授权范围内 4 文件 7 行；**范围外 5 文件 5 行已上报**（`loan-web-ui:175`、`loan-biz-id:80`、`loan-gateway-auth:99`、`loan-web-components:222`、`loan-mini-ui:128`）<br>④ 全量自检：21 个技能 `name` == 目录名、frontmatter 完整、Step 0 门禁在位（✅ 21/21） | team-lead 裁决（2026-08-31） |<br>① 全文清理：台账条数 / 最大编号 / 新增编号起点（`D14`）→ 改为「当前最大编号 + 1」+ 取值命令；表数 68 → 改为「以 `db/loan-db-schema.sql` 为准」不写数字；渠道 tabBar `4 tab` → 改为「以 `TabBar.vue` 为准」不写数量；目录树中 `68 表口径`、`3→4` 等同步清理<br>② §A.2 新增**原则 6** 完整收录规则 9（含 4 类禁止项与取值命令对照表）<br>③ **提出规则 9 的例外**：历史锚点（D8 恢复基线 `b80c965a`、D13 纠错证据「01 写 3 tab 实测 4 tab」、已执行动作记录）**必须保留原值**，否则会自伤——把 `b80c965a` 抽象成"最新 HEAD"将直接摧毁 D8 的删除恢复能力<br>④ §B.3.3 `doc-hygiene` 技能新增规则 9 条目<br>⑤ **发现并上报**：已生成的 4 个技能仍硬编码 `68 表`（`loan-database:102`、`loan-knowledge:204`、`loan-backend:122`、`loan-web-dev:112`），属 T01 产出，已上报待修 | team-lead 消息：写入协议补充第 9 条（2026-08-31） |
