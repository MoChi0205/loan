...
1. `db/loan-db-schema.sql` —— 数据契约。**表数唯一真源**，取值命令：`grep -c "CREATE TABLE" db/loan-db-schema.sql`。
   表数会随 schema 演进而变，**勿写死**。但**下列三类禁止抽象化，必须保留原值**（规则 9 例外条款）：
   ① **恢复锚点**（git 基线 hash、备份路径等）——抽象成「最新 HEAD」会导致删除后无法回滚；
   ② **纠错证据**（「某文档写 X 是错的、实测为 Y」）——抽象掉结论就没了证据；
   ③ **已执行动作记录**（归档了几个文件、修复了哪次重号、改了哪几行）。
   **判据**：改这个值，读者是「取到更新的值」还是「永远丢失一个不可逆事实」？前者抽象，后者保留。拿不准就问用户。
