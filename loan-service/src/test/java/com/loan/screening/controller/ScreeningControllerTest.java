...
class ScreeningControllerTest {
    ...
    void setUp() {
        ...
        screeningService = Mockito.mock(ScreeningService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        ScreeningController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : ScreeningController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(ScreeningController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (ScreeningController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (ScreeningController) ScreeningController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
    ...
    @Test
    @DisplayName("POST /api/admin/screening/run [auth]")
    void post_api_admin_screening_run() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(post("/api/admin/screening/run").content("{}").contentType(MediaType.APPLICATION_JSON))
