 ...
 */
class SensitiveViewControllerTest {

    private MockMvc mvc;
    private SensitiveViewService sensitiveViewService;

    @BeforeEach
    void setUp() {
        // 1) 每个依赖创建深桩 mock（返回安全默认值，避免 NPE）
        sensitiveViewService = Mockito.mock(SensitiveViewService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        SensitiveViewController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : SensitiveViewController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(SensitiveViewController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (SensitiveViewController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (SensitiveViewController) SensitiveViewController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // 3) 字段注入兜底（@Resource/@Autowired 字段）
        ReflectionTestUtils.setField(controller, "sensitiveViewService", sensitiveViewService);
    ...
    @Test
    @DisplayName("POST /api/admin/lead/sensitive/apply-view [auth]")
    void post_api_admin_lead_sensitive_apply_view() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(post("/api/admin/lead/sensitive/apply-view").content("{}").contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    @DisplayName("GET /api/admin/lead/sensitive/quota [auth]")
    void get_api_admin_lead_sensitive_quota() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(get("/api/admin/lead/sensitive/quota"))
