...
class DictControllerTest {
    ...
    void setUp() {
        ...
        dictService = Mockito.mock(DictService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        DictController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : DictController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(DictController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (DictController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (DictController) DictController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // 3) 字段注入兜底（@Resource/@Autowired 字段）
        ReflectionTestUtils.setField(controller, "dictService", dictService);
    ...
    @Test
    @DisplayName("GET /api/dict/all")
    void get_api_dict_all() throws Exception {
        mvc.perform(get("/api/dict/all"))
