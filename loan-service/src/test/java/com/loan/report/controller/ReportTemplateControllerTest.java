...
/**
 * L1 接口契约测试（自动生成，共 3 端点，其中 2 个需登录）。
...
class ReportTemplateControllerTest {
    ...
    void setUp() {
        ...
        templateService = Mockito.mock(ReportTemplateService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        ReportTemplateController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : ReportTemplateController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(ReportTemplateController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (ReportTemplateController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (ReportTemplateController) ReportTemplateController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
    ...
    @Test
    @DisplayName("GET /api/admin/report/template/page")
    void get_api_admin_report_template_page() throws Exception {
        mvc.perform(get("/api/admin/report/template/page"))
            .andExpect(result -> { int s = result.getResponse().getStatus(); if (s >= 500) throw new AssertionError("HTTP status >= 500: " + s); });
    }

    @Test
    @DisplayName("POST /api/admin/report/template/save [auth]")
    void post_api_admin_report_template_save() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(post("/api/admin/report/template/save").content("{}").contentType(MediaType.APPLICATION_JSON))
                .andExpect(result -> { int s = result.getResponse().getStatus(); if (s >= 500) throw new AssertionError("HTTP status >= 500: " + s); });
        } finally {
            UserContext.clear();
        }
    ...
    @Test
    @DisplayName("POST /api/admin/report/template/toggle [auth]")
    void post_api_admin_report_template_toggle() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(post("/api/admin/report/template/toggle").content("{}").contentType(MediaType.APPLICATION_JSON))
                .andExpect(result -> { int s = result.getResponse().getStatus(); if (s >= 500) throw new AssertionError("HTTP status >= 500: " + s); });
        } finally {
            UserContext.clear();
        }
