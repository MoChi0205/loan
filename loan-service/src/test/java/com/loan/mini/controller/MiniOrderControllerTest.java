...
class MiniOrderControllerTest {
    ...
    void setUp() {
        ...
        miniOrderService = Mockito.mock(MiniOrderService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        MiniOrderController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : MiniOrderController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(MiniOrderController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (MiniOrderController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (MiniOrderController) MiniOrderController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // 3) 字段注入兜底（@Resource/@Autowired 字段）
        ReflectionTestUtils.setField(controller, "miniOrderService", miniOrderService);
    ...
    @Test
    @DisplayName("GET /api/mini/order/list [auth]")
    void get_api_mini_order_list() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(get("/api/mini/order/list"))
    ...
    @Test
    @DisplayName("GET /api/mini/order/test [auth]")
    void get_api_mini_order_test() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(get("/api/mini/order/test"))
