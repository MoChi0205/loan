...
/**
 * L1 接口契约测试（自动生成，共 3 端点，其中 3 个需登录）。
 ...
 */
class MiniInvitationControllerTest {

    private MockMvc mvc;
    private InvitationService invitationService;
    private MiniInvitationService miniInvitationService;

    @BeforeEach
    void setUp() {
        // 1) 每个依赖创建深桩 mock（返回安全默认值，避免 NPE）
        invitationService = Mockito.mock(InvitationService.class, new SafeDefaultAnswer());
        miniInvitationService = Mockito.mock(MiniInvitationService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        MiniInvitationController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : MiniInvitationController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(MiniInvitationController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (MiniInvitationController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (MiniInvitationController) MiniInvitationController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
        ...
        ReflectionTestUtils.setField(controller, "miniInvitationService", miniInvitationService);
    ...
    @Test
    @DisplayName("POST /api/mini/invitation/bind [auth]")
    void post_api_mini_invitation_bind() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(post("/api/mini/invitation/bind").content("{}").contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    @DisplayName("GET /api/mini/invitation/mine [auth]")
    void get_api_mini_invitation_mine() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(get("/api/mini/invitation/mine"))
    ...
    @Test
    @DisplayName("GET /api/mini/invitation/records [auth]")
    void get_api_mini_invitation_records() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(get("/api/mini/invitation/records"))
                .andExpect(result -> { int s = result.getResponse().getStatus(); if (s >= 500) throw new AssertionError("HTTP status >= 500: " + s); });
        } finally {
            UserContext.clear();
        }
