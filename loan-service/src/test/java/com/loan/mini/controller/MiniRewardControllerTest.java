 ...
 */
class MiniRewardControllerTest {

    private MockMvc mvc;
    private MiniRewardService miniRewardService;

    @BeforeEach
    void setUp() {
        // 1) 每个依赖创建深桩 mock（返回安全默认值，避免 NPE）
        miniRewardService = Mockito.mock(MiniRewardService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        MiniRewardController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : MiniRewardController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(MiniRewardController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (MiniRewardController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (MiniRewardController) MiniRewardController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // 3) 字段注入兜底（@Resource/@Autowired 字段）
        ReflectionTestUtils.setField(controller, "miniRewardService", miniRewardService);
    ...
    @Test
    @DisplayName("GET /api/mini/reward/mine/summary [auth]")
    void get_api_mini_reward_mine_summary() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(get("/api/mini/reward/mine/summary"))
    ...
    @Test
    @DisplayName("GET /api/mini/reward/mine [auth]")
    void get_api_mini_reward_mine() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(get("/api/mini/reward/mine"))
