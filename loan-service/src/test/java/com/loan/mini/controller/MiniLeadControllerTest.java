 ...
 */
class MiniLeadControllerTest {

    private MockMvc mvc;
    private MiniLeadService miniLeadService;

    @BeforeEach
    void setUp() {
        // 1) 每个依赖创建深桩 mock（返回安全默认值，避免 NPE）
        miniLeadService = Mockito.mock(MiniLeadService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        MiniLeadController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : MiniLeadController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(MiniLeadController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (MiniLeadController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (MiniLeadController) MiniLeadController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // 3) 字段注入兜底（@Resource/@Autowired 字段）
        ReflectionTestUtils.setField(controller, "miniLeadService", miniLeadService);
    ...
    @Test
    @DisplayName("POST /api/mini/lead/submit [auth]")
    void post_api_mini_lead_submit() throws Exception {
        try {
