...
/**
 * L1 接口契约测试（自动生成，共 2 端点，其中 0 个需登录）。
...
class SmsControllerTest {
    ...
    void setUp() {
        ...
        smsService = Mockito.mock(SmsService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        SmsController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : SmsController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(SmsController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (SmsController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (SmsController) SmsController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
    ...
    @Test
    @DisplayName("POST /api/sms/send-code")
    void post_api_sms_send_code() throws Exception {
        mvc.perform(post("/api/sms/send-code").content("{}").contentType(MediaType.APPLICATION_JSON))
            .andExpect(result -> { int s = result.getResponse().getStatus(); if (s >= 500) throw new AssertionError("HTTP status >= 500: " + s); });
    }

    @Test
    @DisplayName("POST /api/sms/verify-code")
    void post_api_sms_verify_code() throws Exception {
        mvc.perform(post("/api/sms/verify-code").content("{}").contentType(MediaType.APPLICATION_JSON))
            .andExpect(result -> { int s = result.getResponse().getStatus(); if (s >= 500) throw new AssertionError("HTTP status >= 500: " + s); });
    }
}
