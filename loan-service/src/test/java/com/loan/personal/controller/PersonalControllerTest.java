...
/**
 * L1 接口契约测试（自动生成，共 2 端点，其中 1 个需登录）。
...
class PersonalControllerTest {
    ...
    void setUp() {
        ...
        personalProfileService = Mockito.mock(PersonalProfileService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        PersonalController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : PersonalController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(PersonalController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (PersonalController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (PersonalController) PersonalController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // 3) 字段注入兜底（@Resource/@Autowired 字段）
        ReflectionTestUtils.setField(controller, "personalProfileService", personalProfileService);
    ...
    @Test
    @DisplayName("POST /api/mini/auth/personal [auth]")
    void post_api_mini_auth_personal() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(post("/api/mini/auth/personal").content("{}").contentType(MediaType.APPLICATION_JSON))
    ...
    @Test
    @DisplayName("GET /api/admin/personal/test")
    void get_api_admin_personal_test() throws Exception {
        mvc.perform(get("/api/admin/personal/test"))
