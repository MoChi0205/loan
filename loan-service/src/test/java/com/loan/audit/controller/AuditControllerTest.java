...
class AuditControllerTest {
    ...
    void setUp() {
        ...
        auditService = Mockito.mock(AuditService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        AuditController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : AuditController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(AuditController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (AuditController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (AuditController) AuditController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
    ...
    @Test
    @DisplayName("GET /api/admin/audit/test")
    void get_api_admin_audit_test() throws Exception {
        mvc.perform(get("/api/admin/audit/test"))
            .andExpect(status().is2xxSuccessful()).andExpect(jsonPath("$.code").exists());
    }

    @Test
    @DisplayName("GET /api/admin/audit/page")
    void get_api_admin_audit_page() throws Exception {
        mvc.perform(get("/api/admin/audit/page"))
