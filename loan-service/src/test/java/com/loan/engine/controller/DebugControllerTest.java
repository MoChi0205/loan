...
class DebugControllerTest {
    ...
    void setUp() {
        ...
        planLoaderService = Mockito.mock(PlanLoaderService.class, new SafeDefaultAnswer());
        matchService = Mockito.mock(MatchService.class, new SafeDefaultAnswer());
        matchTraceMapper = Mockito.mock(MatchTraceMapper.class, new SafeDefaultAnswer());
        matchRuleLogMapper = Mockito.mock(MatchRuleLogMapper.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        DebugController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : DebugController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(DebugController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (DebugController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (DebugController) DebugController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // 3) 字段注入兜底（@Resource/@Autowired 字段）
        ReflectionTestUtils.setField(controller, "planLoaderService", planLoaderService);
        ReflectionTestUtils.setField(controller, "matchService", matchService);
        ReflectionTestUtils.setField(controller, "matchTraceMapper", matchTraceMapper);
        ReflectionTestUtils.setField(controller, "matchRuleLogMapper", matchRuleLogMapper);
    ...
    @Test
    @DisplayName("POST /api/debug/match")
    void post_api_debug_match() throws Exception {
        mvc.perform(post("/api/debug/match").content("{}").contentType(MediaType.APPLICATION_JSON))
