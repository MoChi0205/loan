 ...
 */
class BankChannelControllerTest {
    ...
    void setUp() {
        ...
        bankChannelMapper = Mockito.mock(BankChannelMapper.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        BankChannelController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : BankChannelController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(BankChannelController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (BankChannelController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (BankChannelController) BankChannelController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // 3) 字段注入兜底（@Resource/@Autowired 字段）
        ReflectionTestUtils.setField(controller, "bankChannelMapper", bankChannelMapper);
    ...
    @Test
    @DisplayName("GET /api/admin/channel/list")
    void get_api_admin_channel_list() throws Exception {
        mvc.perform(get("/api/admin/channel/list"))
