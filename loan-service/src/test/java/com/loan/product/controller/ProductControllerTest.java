...
class ProductControllerTest {
    ...
    void setUp() {
        ...
        productQueryService = Mockito.mock(ProductQueryService.class, new SafeDefaultAnswer());
        productService = Mockito.mock(ProductService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        ProductController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : ProductController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(ProductController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (ProductController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (ProductController) ProductController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
            throw new RuntimeException(e);
        }
        // 3) 字段注入兜底（@Resource/@Autowired 字段）
        ReflectionTestUtils.setField(controller, "productQueryService", productQueryService);
    ...
    @Test
    @DisplayName("GET /api/admin/product/page [auth]")
    void get_api_admin_product_page() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(get("/api/admin/product/page"))
    ...
    @Test
    @DisplayName("GET /api/admin/product/test")
    void get_api_admin_product_test() throws Exception {
        mvc.perform(get("/api/admin/product/test"))
            .andExpect(status().is2xxSuccessful()).andExpect(jsonPath("$.code").exists());
    }

    @Test
    @DisplayName("DELETE /api/admin/product/test [auth]")
    void delete_api_admin_product_test() throws Exception {
        try {
            UserContext.setUser(TestUsers.staffUser());
            mvc.perform(delete("/api/admin/product/test"))
                .andExpect(result -> { int s = result.getResponse().getStatus(); if (s >= 500) throw new AssertionError("HTTP status >= 500: " + s); });
        } finally {
            UserContext.clear();
        }
