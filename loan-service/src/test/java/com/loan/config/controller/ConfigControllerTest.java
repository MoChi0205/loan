...
class ConfigControllerTest {
    ...
    void setUp() {
        ...
        configService = Mockito.mock(ConfigService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        ConfigController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : ConfigController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(ConfigController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (ConfigController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (ConfigController) ConfigController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
    ...
    @Test
    @DisplayName("GET /api/admin/config/status")
    void get_api_admin_config_status() throws Exception {
        mvc.perform(get("/api/admin/config/status"))
