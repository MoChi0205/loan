...
class OcrControllerTest {
    ...
    void setUp() {
        ...
        ocrService = Mockito.mock(OcrService.class, new SafeDefaultAnswer());
        // 2) 构造控制器（优先构造函数，否则无参 + 字段注入兜底）
        OcrController controller;
        try {
            Constructor<?> ctor = null;
            for (Constructor<?> c : OcrController.class.getDeclaredConstructors()) {
                if (c.isAnnotationPresent(org.springframework.beans.factory.annotation.Autowired.class)) { ctor = c; break; }
            }
            if (ctor == null) {
                ctor = Arrays.stream(OcrController.class.getDeclaredConstructors())
            ...
            if (ctor != null && ctor.getParameterCount() > 0) {
                ...
                controller = (OcrController) ctor.newInstance(args);
            } else {
                // 反射式无参实例化（编译期不依赖无参构造器存在）
                controller = (OcrController) OcrController.class.getDeclaredConstructor().newInstance();
            }
        } catch (Exception e) {
    ...
    @Test
    @DisplayName("GET /api/admin/ocr/field-defs")
    void get_api_admin_ocr_field_defs() throws Exception {
        mvc.perform(get("/api/admin/ocr/field-defs"))
            .andExpect(result -> { int s = result.getResponse().getStatus(); if (s >= 500) throw new AssertionError("HTTP status >= 500: " + s); });
    }

    @Test
    @DisplayName("POST /api/admin/ocr/record")
    void post_api_admin_ocr_record() throws Exception {
        mvc.perform(post("/api/admin/ocr/record").content("{}").contentType(MediaType.APPLICATION_JSON))
            .andExpect(result -> { int s = result.getResponse().getStatus(); if (s >= 500) throw new AssertionError("HTTP status >= 500: " + s); });
    }
}
