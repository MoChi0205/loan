...
/**
 * 「我的报告」子视图（员工侧由 pages/client/mine.vue 容器承载，客户端为独立 tab 页）。
 *
 * C3 角色二分 + C11 四维查询：
 ...
 * {@code loadMore} / {@code refresh}（defineExpose）；底部导航由容器页提供。
 */
const PAGE_SIZE = 10;
...
/** 首屏 / 重新激活：拉取第一页（渠道守卫在容器页与入口层完成，D50） */
function activate() {
...
/** 下拉刷新：由容器页 onPullDownRefresh 转发调用 */
