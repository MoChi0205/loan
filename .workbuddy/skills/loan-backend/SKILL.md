...
- **审批权限真值（D60 覆盖 D59 的 PRODUCT 部分）**：`ALLOCATION` 审批包含 `DEPT_MANAGER`，但部门经理仅可审批本人团队；渠道 `PRODUCT` 录入/删除终审仅 `BOSS/SUPER_ADMIN/SUPER`；`DOWNLOAD/MATERIAL_REVIEW` 可包含 `DEPT_MANAGER/OPERATOR`。
