...
- ✅ **可审批 `PRODUCT` / `DOWNLOAD` / `MATERIAL_REVIEW`** —— `APPROVAL_ROLES = [BOSS, DEPT_MANAGER, OPERATOR, SUPER_ADMIN, SUPER]` **含 BOSS**
...
> 当前白名单 `loan.mini.approval.types=ALLOCATION,MATERIAL_REVIEW,PRODUCT,DOWNLOAD`（`application.properties:110`）。
> —— 四类审批均已开放；渠道 PRODUCT 终审仅 BOSS/SUPER_ADMIN/SUPER，BOSS 可审全公司 ALLOCATION。
