...
- 查看公司已生效的银行产品（只读，不含客户匹配结果与内部编码）
- 分享邀请客户；分享链接或二维码自动携带邀请码
- **我的产品（自有录入）**：`GET /api/mini/product/list`（后端 `requireChannel`）、`POST /api/mini/product`（录入 DRAFT）、
  ...
  —— 产品编码由后端生成、所属银行由登录态推导；录入和删除均由老板/超级管理员终审，删除复用
  `t_product_approval`（`applyType=CREATE/DELETE`，C9，**不新增表**）；通过录入审批后自动幂等进入合作库
...
5. ❌ **录入命中全库硬查重时，只返友好文案，严禁泄露客户编码、归属人及是否可认领** —— 渠道沙箱只提示已存在
...
| `POST /api/mini/product` | 录入产品（DRAFT，内部编码后端生成） | **仅 CHANNEL**（KB 旧文「CHANNEL/STAFF」不实） |
...
| `POST /api/mini/product/{code}/delete-apply` | 申请删除 | 仅本人（APPROVED；兼容历史 OK） |
...
> `POST /api/mini/partner-product/delete/{approvalNo}/audit`，权限是**老板 / 超管**，**不是渠道或运营**；
> 通过后合作库置 `OFFLINE`，审批记录永久保留。

> 渠道产品录入审批同样属于渠道新增内容终审，PRODUCT 的审核动作仅老板 / 超级管理员可执行；
> 部门经理和运营不应展示操作按钮，后端 `MiniRoleGuard` 仍会二次拒绝越权请求。
