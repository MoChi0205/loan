...
description: >-
  ...
  涉及运营角色的需求评审、ALLOCATION/DOWNLOAD/MATERIAL_REVIEW 审批、审批中心入口、
...
- ✅ 可审批 `ALLOCATION`、`DOWNLOAD`、`MATERIAL_REVIEW`；渠道 `PRODUCT` 录入/删除终审仅老板/超级管理员，运营不可审（D60）
...
|---|---|
| 报告 / 工单 / 客户 | **全公司全量** |
| 审批 | ALLOCATION + DOWNLOAD + MATERIAL_REVIEW；不含渠道 PRODUCT |
| 配置域 | ✅ 可见可改（**Web 管理端**） |
| 审计 / 短信 / 模板 | ✅（Web 管理端） |
...
| `POST /api/mini/approval/{type}/{approvalNo}/audit` | 审批（body `{approve, opinion}`） | PRODUCT 被拒绝 |
...
> 当前四类白名单均开放，但白名单不替代角色守卫；运营的 PRODUCT 计数为 0、列表不返回、动作被拒绝。
