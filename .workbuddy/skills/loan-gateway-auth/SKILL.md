...
- **审批接口权限真值（D60）**：`ALLOCATION` 包含 `DEPT_MANAGER` 但仅本人团队；渠道 `PRODUCT` 录入/删除终审仅 `BOSS/SUPER_ADMIN/SUPER`；`DOWNLOAD/MATERIAL_REVIEW` 可包含 `DEPT_MANAGER/OPERATOR`。网关与应用守卫必须一致。
